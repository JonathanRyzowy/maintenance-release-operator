...
2. **Automated PR Checks**
   ...
   - Status: `success` (exit code 0) or `failure` (exit code 1)
...
2. **Check Runner**
   ...
   - Runs `npx maintenance-release-operator check --json`
   - Parses JSON output
...
4. **Billing Integration (Future)**
   ...
   - Enforce tier limits (5 repos for Starter, 50 for Organization)
...
- Week 2: Check Run posting, error handling, database schema
- Week 3: Testing on multiple repos, edge cases, logging
- Week 4: Polish, documentation, soft launch (free tier)

**Full-time:** 1–2 weeks for MVP.
...
3. **Repo Cloning**
   - Use shallow clones (`--depth 1`) to minimize data transfer
...
- FAQ for common issues (e.g., "Why did my PR fail?")
...
1. **Register GitHub App** (requires GitHub account with org admin access)
2. **Deploy webhook receiver** (Vercel/Railway/Fly.io)
3. **Test MVP on personal org**
4. **Soft launch to early adopters** (open GitHub issue with tag `github-app-early-access`)
5. **Iterate based on feedback**
6. **Launch paid tiers** (after Stripe integration)
